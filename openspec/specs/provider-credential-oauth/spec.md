...
Defines runtime credential modeling, credential-aware provider auth selection,
OAuth bearer support for Kimi Code and Codex, Codex request behavior, and the
boundary that keeps OAuth refresh lifecycle ownership outside this crate.
...
- **AND** the profile uses `ApiFamily::Responses`
...
- **WHEN** `ProviderConnection` dispatches a non-streaming request for the Codex Responses-family profile
...
- **WHEN** `ProviderConnection` dispatches a streaming request for the Codex Responses-family profile
- **THEN** it sends `POST https://chatgpt.com/backend-api/codex/responses`
